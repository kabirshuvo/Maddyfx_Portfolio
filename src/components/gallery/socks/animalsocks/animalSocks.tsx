...
const AnimalSocks: React.FC = () => {
  const [selectedImage, setSelectedImage] = useState<AnimalSocksDesign | null>(
    null
  );

  const handleImageClick = (design: AnimalSocksDesign) => {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-col justify-center items-center bg-emerald-700 h-80">
        <h1 className="text-5xl text-white">Animal Designed Socks</h1>
        <hr />
        <br />
        <GalleryNavBar />
      </div>
      <div className="columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4 mx-auto p-4 -mt-24">
        {AnimakSockDesigns.map((design) => (
          <div
            key={design.id}
      ...
      {selectedImage && (
        <Modal
          onClose={closeModal}
          images={AnimakSockDesigns}
          initialIndex={AnimakSockDesigns.findIndex(
