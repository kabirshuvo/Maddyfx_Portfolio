...
const KnittingSocks: React.FC = () => {
  const [selectedImage, setSelectedImage] =
    useState<KnittingSockDesignImages | null>(null);

  const handleImageClick = (design: KnittingSockDesignImages) => {
  ...
  return (
    <div className="flex flex-col">
      <div className="flex flex-col justify-center items-center bg-emerald-700 h-80">
        <h1 className="text-5xl text-white">Knitting Socks</h1>
        <hr />
        <br />
        <GalleryNavBar />
      </div>
      <div className="columns-1 md:columns-2 lg:columns-3 gap-4 space-y-4 mx-auto p-4 -mt-24">
        {KnittingSockinitialDesigns.map((design) => (
          <div
            key={design.id}
      ...
      {selectedImage && (
        <Modal
          onClose={closeModal}
          images={KnittingSockinitialDesigns}
          initialIndex={KnittingSockinitialDesigns.findIndex(
